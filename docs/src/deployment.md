...
Every LMDB-backed file — data, dictionaries, indexes, the VOC — now
lives in the daemon; directory files (BP source) stay local. No
application change, no relink: the `lmdbnet` driver presents the
identical contract over the wire.
